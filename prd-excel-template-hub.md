...
*v2.0 — cập nhật theo hướng đi thực tế*

| Version | v2.0 |
| :---- | :---- |
| **Ngày** | 28/07/2026 |
| **Thay thế** | v1.0 (25/03/2026) |
...
## 0. NHỮNG GÌ ĐÃ THAY ĐỔI SO VỚI v1.0
...
Google search "mẫu excel tính lương"
  ...
  Nhận ra "mình chưa tự làm được cái này"
        ...
        │
        └──► /khoa-hoc-excel  ──► form lead ──► webhook (Sheets/Zapier)
                    │
                    └──► /khoa-hoc-excel/cam-on ──► link sang HVS
```
...
Mỗi trang template link ra:             ▼
  ...
  · CTA         → HVS (kèm UTM) + /khoa-hoc-excel
```
...
| Structured data | CreativeWork + BreadcrumbList + FAQPage trên mỗi trang template |
| Lead | `app/api/lead/route.ts` → `LEAD_WEBHOOK_URL`; chưa cấu hình thì ghi log để không mất lead |
| Analytics | GA4 qua `NEXT_PUBLIC_GA_ID` |
| Deploy | Vercel (bản thật) · GitHub Pages `NEXT_PUBLIC_PREVIEW=1` (export tĩnh, xóa `app/api`, chặn index) |
...
Chỉnh lại theo mô hình bridge: ta **không** đo được chuyển đổi cuối trên HVS, nên thước đo thật là lead form và click có UTM.
...
| Click CTA sang HVS | GA4 outbound + UTM đối chiếu với HVS | > 3% |
| **Lead form gửi thành công** | Webhook / Google Sheets | **> 1%** lượt xem `/khoa-hoc-excel` |
| Thời gian trên trang | GA4 | > 90 giây |

*UTM bắt buộc trên mọi link ra HVS: `utm_content` = slug template, để biết chính xác trang nào ra lead chứ không chỉ biết "từ site template".*
...
### Đã xong
Toàn bộ hạ tầng: 3 loại trang template, SSG, meta động, canonical khớp trailing slash, JSON-LD ba loại, breadcrumb, related tự bù, sitemap lọc category rỗng, robots, download .xlsx, CTA có UTM, trang khóa học + form lead + trang cảm ơn, GA4 component, pipeline validate/build/QA, CI deploy GitHub Pages.
...
| 3 | Cấu hình `LEAD_WEBHOOK_URL` | Chưa có → lead chỉ nằm trong console log |
